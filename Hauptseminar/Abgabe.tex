...
Diese Methode verwendet zwei Eingaben und eine Ausgabe und zusätzlich ein Taktsignal. Hierbei werden dem Zyklus immer jeweils ein Bit der Eingaben zugeführt. Typischerweise vom least-significant Bit bis zum most-significant BIt. Während jedem Takt wird ein Teilergebnis zu einer Gesamtsumme addiert und das least-significant Bit der Gesamtsumme wird in die Ausgabe verlagert. \\
...
Unter Zuhilfenahme eines  fest zugeordnetem traditionellen Sammler kann ein hybrid aus seriellem und parallelem Multiplizierer aufgebaut werden. Dabei wird ein Operand seriell und ein Operand parallel bereitgestellt. Der parallele Operand muss dabei über die gesamte Dauer konstant bleiben. Der serielle Operand wir dagegen Bitweise vom least-significant Bit zum most-significant Bit übergeben. Dabei wird jedes Bit untersucht. Ist es eine Eins, so wird dem mit Null initialisiertem Sammler der parallele Operand hinzu addiert. Bei einer Null wird keine Addition ausgeführt. Nach diesem Schritt wird das untersuchte Bit aus dem Multiplikationszyklus ausgelagert. Dieser Prozess wird für alle 2n+1 Bit des Produkts ausgeführt.\\
...
So gesehen ist der Algorithmus sicher, wenn Angreifer eine verschlüsselte Nachrichten abfangen und versuchen diese zu entschlüsseln. Doch falls ein Angreifer zudem Zugang zu der physikalischen Schicht des Systems hat, eröffnet ihm das neue Möglichkeiten, vor allem durch anwendung sogenannter 'side-channel Attacken'. Diese Angriffe richten sich auf ungewollte Informationslecks., vor allem auf die zeitliche Aktivität und den Stromverbrauch der Hardware. Die Techniken die diese Channels analysieren haben ähnliche Konzepte und brauchen nicht zwingend Systemzugriff.\\
Angenommen ein Angreifer weis welches Verfahren der Hardware Implementierung angewandt wurde, in Annhame den serielI/parallel Multiplizierer. Dieses System errechnet den Chiffretext durch Quadrieren des Normaltextes und anschließender Subtraktion des Modulus von N. Dieser Prozess wird E  - 1 mal wiederholt. Eventuell wurde das System so entworfen, dass der das zuvor errechnete Produkt Teil der parallelen Multiplikation ist, wärend der Normaltext der Operand der seriellen Multiplikation ist. Daraus folgt, das für die Berechnung des Chiffretextes C der Normaltext N E mal seriell in den Muliplizierer gespeist wird. Hier wird jedes Bit des seriellen Operanten untersucht und dem Speicher hinzu addiert. Wenn das aktuelle Bit 1 ist, wird die Addition durchgeführt, falls es 0 ist wird keine Addition durchgeführt. Aus dem algorithmischen Standpunkt aus ist diese Entscheidung irrelevant, für die Zeti und Strom Chnnels hat dies jedoch Auswirkungen. So hat etwa die Durchführung der Addition eine längere Zeit oder einen höheren Stromverbrauch zur Folge. In jedem Fall kann ein Angreifer diese Abweichungen messen. Solche Informationen lassen Rückschlüsse auf die geheime Berechnung zu. Einfache änderungen in dem System könnten solche Attacken zu Nichte machen. Etwa wird immer eine Addition durchgeführt, das Ergebnis gegebenen Falls aber verworfen. \\
Der hybrid Multiplizierer ist jedoch nicht das einzige System, dass solche Informationen preis gibt. Der kombinatorische Multiplizierer mit seiner Größe macht ihn Anfällig für side-channel Attacken. Hier können durch die hohen Durchlaufzahlen und der daraus resultierenden Temperatur Rückschlüsse auf welche Zeilen und Spalten beteiligt waren.\\
Eine vollständige Sicherheit kann also nur erreicht werden, wenn sowohl auf die korrekte Ausführung des Algorithmus achtgegeben wird, als auch auf eine Implementierung, die kaum Rückschlüsse auf die Berechnung gibt.



\subsection{Fazit}
Seit seiner Entwicklung 1979 hat sich am Prinzip der RSA Verschlüsselung nichts geändert. Mit dem Vortschritt der Technik ist der RSA Standartteil der sicheren Kommunikation georden. Aktuelle Rechner können die Schlüsselpaare angeboren berechnen. Doch so leicht es auch ist, den Algorithmus mathematisch zu bestimmen, so beängstigend ist es ihn hardware-spezifisch umzusetzen. Dieser Algorithmus verändert sich ständig durch kliene Änderungen weiter. Wie zum Beispiel die Verwendung einer Fermat Primzahl als Verschlüsselungs-Exponenten. Andere Änderungen wie den Ausschluss der side-channel Attacken bedarf etwaiger Änderungen, die eventuell großen Einfluss auf die Komplexität, den Platz oder den Stromverbrauch des Systems haben. Wie bei jeder anderen Verschlüsselung ist das Ziel der Ausschluss derer, die sie knacken wollen um die sichere Kommunikation zwischen zwei Partnern zu gewährleisten. 
